...
local RayfieldLibrary = {
	...
	Theme = {
		Default = {
			BackgroundImage = "rbxassetid://11878291121",
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateParagraph(ParagraphSettings)
			...
			function ParagraphValue:CreateCircularProgress(settings)
				local container = Instance.new("Frame")
				container.Size = UDim2.new(0, 60, 0, 60)
				container.Position = UDim2.new(1, -70, 0, 10)
				...
				image.Image = "rbxassetid://266748367"
				image.Parent = container
				
				local text = Instance.new("TextLabel")
				text.Size = UDim2.new(1, 0, 1, 0)
				text.BackgroundTransparency = 1
				text.Text = tostring(settings.InitialValue or 0) .. "%"
				...
				local function update(val)
					text.Text = tostring(math.clamp(val, 0, 100)) .. "%"
				end
				return { Update = update }
			end

			function ParagraphValue:CreateNotificationHistory(settings)
				...
				local history = {}
				function history:Add(msg, typ)
					local label = Instance.new("TextLabel")
					label.Size = UDim2.new(1, 0, 0, 20)
					label.BackgroundColor3 = Color3.fromRGB(40,40,40)
					label.Text = "[" .. os.date("%H:%M:%S") .. "] " .. msg
					label.TextColor3 = (typ == "Error" and Color3.new(1,0,0)) or Color3.new(1,1,1)
			...
			function ParagraphValue:CreateSearchDropdown(settings)
				local btn = Instance.new("TextButton")
				btn.Size = UDim2.new(1, 0, 0, 30)
				btn.Position = UDim2.new(0, 0, 1, 10)
				btn.Text = settings.Default or "Select..."
				btn.BackgroundColor3 = Color3.fromRGB(50,50,50)
				btn.Parent = Paragraph
			...
			--[[function ParagraphValue:CreateScriptExecutor(settings)
				...
				box.Parent = Paragraph
				
				local exec = Instance.new("TextButton")
				exec.Size = UDim2.new(0.5, 0, 0, 25)
				exec.Position = UDim2.new(0, 0, 1, 100)
				exec.Text = "Execute"
				exec.BackgroundColor3 = Color3.fromRGB(0,150,0)
				exec.Parent = Paragraph
			...
			function ParagraphValue:CreateMouseTracker(settings)
				...
				label.BackgroundColor3 = Color3.fromRGB(0,0,0,0.5)
				label.Text = "X: 0, Y: 0"
				...
				local conn = uis.InputChanged:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseMovement then
						label.Text = string.format("X: %.0f, Y: %.0f", input.Position.X, input.Position.Y)
					end
				end)
      ...
      function Tab:CreateDropdown(DropdownSettings)
			...
			local function SetDropdownOptions()
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if DropdownSettings.LinkedSlider then
							local val = nil
							if DropdownSettings.LinkedSliderValueMap then
								val = DropdownSettings.LinkedSliderValueMap[Option]
							else
								val = tonumber(Option)
							end
							if val and DropdownSettings.LinkedSlider.SetSilent then
								DropdownSettings.LinkedSlider:SetSilent(val)
							elseif DropdownSettings.LinkedSlider.Set then
								DropdownSettings.LinkedSlider:Set(val)
							end
						end

						if DropdownSettings.LinkedToggle then
							local state = nil
							if DropdownSettings.LinkedToggleValueMap then
								state = DropdownSettings.LinkedToggleValueMap[Option]
							else
								state = Option == "true" or Option == "Enabled" or Option == "On"
							end
							if state ~= nil and DropdownSettings.LinkedToggle.Set then
								DropdownSettings.LinkedToggle:Set(state)
							end
						end
			...
			function DropdownSettings:Set(NewOption)
				...
				if DropdownSettings.LinkedSlider then
					local selectedOpt = DropdownSettings.CurrentOption[1]
					local val = nil
					if DropdownSettings.LinkedSliderValueMap then
						val = DropdownSettings.LinkedSliderValueMap[selectedOpt]
					else
						val = tonumber(selectedOpt)
					end
					if val and DropdownSettings.LinkedSlider.SetSilent then
						DropdownSettings.LinkedSlider:SetSilent(val)
					elseif DropdownSettings.LinkedSlider.Set then
				...
				if DropdownSettings.LinkedToggle then
					...
					local state = nil
					if DropdownSettings.LinkedToggleValueMap then
						state = DropdownSettings.LinkedToggleValueMap[selectedOpt]
					else
						state = selectedOpt == "true" or selectedOpt == "Enabled" or selectedOpt == "On"
					end
					if state ~= nil and DropdownSettings.LinkedToggle.Set then
						DropdownSettings.LinkedToggle:Set(state)
					end
		...
		-- Image dropdown made by TsurenStudios 
	function Tab:CreateImageDropdown(DropdownSettings)
    local Dropdown = Elements.Template.Dropdown:Clone()
    Dropdown.Name = DropdownSettings.Name
    Dropdown.Title.Text = DropdownSettings.Name
    Dropdown.Visible = true
    ...
    local function SetSelectedImage(imageId)
        if Dropdown:FindFirstChild("SelectedImage") then
            Dropdown.SelectedImage.Image = imageId
            Dropdown.SelectedImage.Visible = true
        else
            if Dropdown:FindFirstChild("Selected") then
                Dropdown.Selected.Visible = false
            end
            local img = Instance.new("ImageLabel")
            img.Name = "SelectedImage"
            img.Size = UDim2.new(0, 30, 0, 30)
            img.Position = UDim2.new(0, 10, 0, 7.5)
    ...
    if DropdownSettings.CurrentOption and #DropdownSettings.CurrentOption > 0 then
        local first = DropdownSettings.CurrentOption[1]
        local imageId = type(first) == "table" and first.Image or first
        ...
        if Dropdown.List.Visible then
            ...
            for _, opt in ipairs(Dropdown.List:GetChildren()) do
                if opt.ClassName == "Frame" and opt.Name ~= "Placeholder" then
                    ...
                    if opt:FindFirstChild("Image") then
                        TweenService:Create(opt.Image, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {ImageTransparency = 1}):Play()
                    end
                end
            end
            TweenService:Create(Dropdown.List, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {ScrollBarImageTransparency = 1}):Play()
            ...
            for _, opt in ipairs(Dropdown.List:GetChildren()) do
                if opt.ClassName == "Frame" and opt.Name ~= "Placeholder" then
                    ...
                    if opt:FindFirstChild("Image") then
                        TweenService:Create(opt.Image, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {ImageTransparency = 0}):Play()
                    end
                    if opt.Name ~= (DropdownSettings.CurrentOption[1] and (type(DropdownSettings.CurrentOption[1])=="table" and DropdownSettings.CurrentOption[1].Image or DropdownSettings.CurrentOption[1]) or "") then
    ...
    local function SetDropdownOptions()
        for _, Option in ipairs(DropdownSettings.Options) do
            ...
            if type(Option) == "table" then
                ...
                ImageId = tostring(Option)
            end

            local DropdownOption = Elements.Template.Dropdown.List.Template:Clone()
            DropdownOption.Name = ImageId

            if DropdownOption:FindFirstChild("Title") then
                DropdownOption.Title.Visible = false
            end

            local img = DropdownOption:FindFirstChild("Image")
            if img then
                img.Image = ImageId
                img.Visible = true
                ...
                newImg.Name = "Image"
                newImg.Size = UDim2.new(0, 30, 0, 30)
                newImg.Position = UDim2.new(0, 10, 0.5, -15)
                newImg.BackgroundTransparency = 1
                newImg.Image = ImageId
                newImg.ImageTransparency = 1
                newImg.Parent = DropdownOption
                img = newImg
            end
            ...
            DropdownOption.Interact.ZIndex = 50
            DropdownOption.Interact.MouseButton1Click:Connect(function()
                if not DropdownSettings.MultipleOptions and table.find(DropdownSettings.CurrentOption, ImageId) then
                    return
                end

                if table.find(DropdownSettings.CurrentOption, ImageId) then
                    table.remove(DropdownSettings.CurrentOption, table.find(DropdownSettings.CurrentOption, ImageId))
                    if #DropdownSettings.CurrentOption > 0 then
                        local first = DropdownSettings.CurrentOption[1]
                        local id = type(first) == "table" and first.Image or first
                        SetSelectedImage(id)
                    else
                        if Dropdown:FindFirstChild("SelectedImage") then
                            Dropdown.SelectedImage.Visible = false
                        end
                    end
                else
                    if not DropdownSettings.MultipleOptions then
                    ...
                    table.insert(DropdownSettings.CurrentOption, ImageId)
                    SetSelectedImage(ImageId)
                    TweenService:Create(DropdownOption.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
                    TweenService:Create(DropdownOption, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.DropdownSelected}):Play()
                    Debounce = true
                ...
                if DropdownSettings.LinkedSlider then
                    local val = nil
                    if DropdownSettings.LinkedSliderValueMap then
                        val = DropdownSettings.LinkedSliderValueMap[ImageId]
                    else
                        val = tonumber(ImageId)
                    end
                    if val and DropdownSettings.LinkedSlider.SetSilent then
                        DropdownSettings.LinkedSlider:SetSilent(val)
                    elseif DropdownSettings.LinkedSlider.Set then
                ...
                if DropdownSettings.LinkedToggle then
                    local state = nil
                    if DropdownSettings.LinkedToggleValueMap then
                        state = DropdownSettings.LinkedToggleValueMap[ImageId]
                    else
                        state = ImageId == "true" or ImageId == "Enabled" or ImageId == "On"
                    end
                    if state ~= nil and DropdownSettings.LinkedToggle.Set then
                        DropdownSettings.LinkedToggle:Set(state)
                    end
    ...
    Toggle.Interact.MouseButton1Click:Connect(function()
        if ToggleSettings.ToggleWait == true then 
            if type(ToggleSettings.ToggleClicked) == "function" then
    ...
    return ToggleSettings
		...
		function Tab:CreateSlider(SliderSettings)
			...
			function SliderSettings:SetSilent(NewVal)
				NewVal = math.clamp(NewVal, SliderSettings.Range[1], SliderSettings.Range[2])
				local width = Slider.Main.AbsoluteSize.X * ((NewVal - SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1]))
				width = width > 5 and width or 5
				TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = UDim2.new(0, width, 1, 0)}):Play()
				Slider.Main.Information.Text = tostring(NewVal) .. " " .. (SliderSettings.Suffix or "")
				SliderSettings.CurrentValue = NewVal
				if not SliderSettings.Ext then
					SaveConfiguration()
